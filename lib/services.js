 ...
 * expect, so no presentational component needs changing.
 *
...
function transformService(row) {
  return {
    ...
    durations: (row.service_durations ?? [])
      ...
      .map((d) => ({ id: d.id, minutes: d.minutes, price: d.price })),
    image: row.image_url,
...
const SERVICE_SELECT =
  "*, service_categories(slug, label), service_durations(id, minutes, price, sort_order)";
